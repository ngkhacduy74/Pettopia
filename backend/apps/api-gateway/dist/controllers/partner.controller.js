...
let PartnerController = class PartnerController {
    ...
    async findAllClinic(page = 1, limit = 10, userRole) {
        return await (0, rxjs_1.lastValueFrom)(this.partnerService.send({ cmd: 'findAllClinic' }, {
            page,
            limit,
            isAdmin: [roles_decorator_1.Role.ADMIN, roles_decorator_1.Role.STAFF].includes(userRole),
        }));
    }
    ...
    async getClinicMembers(clinicId, page = 1, limit = 10) {
        ...
        return await (0, rxjs_1.lastValueFrom)(this.partnerService.send({ cmd: 'getClinicMembers' }, {
            clinic_id: clinicId,
            page,
            limit,
        }));
